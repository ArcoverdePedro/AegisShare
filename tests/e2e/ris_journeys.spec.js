...
const LIST = '/imagem/pedidos/';
const CREATE = '/imagem/encontros/06000000-0000-4000-8000-000000000002/pedidos/novo/';
const EXAM = '06000000-0000-4000-8000-000000000003';
...
async function login(page, role = 'operator') {
  ...
  await page.getByLabel('Usuário').fill(`ci-ris-${role}`);
  await page.getByLabel('Senha').fill(`ci-ris-${role}-password`);
  await page.getByRole('button', { name: 'Entrar', exact: true }).click();
  await expect(page).toHaveURL(/\/$/);
}
async function order(page) {
  await page.goto(CREATE);
  await page.getByLabel('Exame de imagem', { exact: true }).selectOption(EXAM);
  await page.getByRole('button', { name: 'Registrar pedido de imagem' }).click();
  await expect(page.getByRole('heading', { name: 'Pedido de imagem' })).toBeVisible();
  return page.url();
}
test('CA-RIS-01/05: pedido e retry preservam protocolo', async ({page}) => {
  await login(page);
  await page.goto(CREATE);
  await page.getByLabel('Exame de imagem', {exact:true}).selectOption(EXAM);
  const data = await page.locator('form.box').evaluate(form=>Object.fromEntries(new FormData(form)));
  await page.getByRole('button', {name:'Registrar pedido de imagem'}).click();
  await expect(page.getByRole('heading',{name:'Pedido de imagem',exact:true})).toBeVisible();
  const detail = page.url();
  const retry = await page.request.post(CREATE, {form:data});
  expect(retry.status()).toBe(200);
  expect(retry.url()).toBe(detail);
  await expect(page.getByText('Situação: Solicitado')).toBeVisible();
});

test('CA-RIS-03: pedido fora do escopo não é revelado', async ({ page }) => {
  await login(page);
  ...
  await expect(page.getByText('Pessoa Sintética RIS')).toHaveCount(0);
});

test('CA-RIS-07: offline não confirma nem guarda pedido', async ({ page, context }) => {
  await login(page);
  ...
  await Promise.all([
    ...
    page.getByRole('button', { name: 'Registrar pedido de imagem' }).click(),
  ]);
  await page.goto(LIST);
    ...
    for (const name of await caches.keys()) {
      for (const request of await (await caches.open(name)).keys()) {
        ...
        if (/text|json|javascript/.test(response.headers.get('content-type') || '')) {
          ...
          sensitive ||= body.includes('Pessoa Sintética RIS') || body.includes('Exame Sintético RIS');
        }
      }
    }
    sensitive ||= JSON.stringify(localStorage).includes('Pessoa Sintética RIS') || JSON.stringify(sessionStorage).includes('Pessoa Sintética RIS');
    return { urls, sensitive, databases: (await indexedDB.databases()).map(db => db.name) };
  });
  expect(storage.sensitive).toBe(false);
  expect(storage.urls.some(url => url.includes('/imagem/'))).toBe(false);
  expect(storage.databases).toEqual(databases);
});

test('CA-RIS-04/07: telas em telefone e tablet sem barreiras graves', async ({ page }) => {
...
test('CA-RIS-02: concessão revogada depois de abrir formulário', async ({page, browser}) => {
  ...
  try {
    ...
    await adminPage.goto('/aegis-admin/pep/patientaccessgrant/06000000-0000-4000-8000-000000000004/delete/');
