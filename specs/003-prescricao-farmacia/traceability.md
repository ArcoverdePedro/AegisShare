...
| **RNF-RX-04 — Minimização de PHI em logs/eventos** | **Parcial** | Eventos atuais têm allowlist técnico; campos textuais sensíveis são excluídos do auditlog onde aplicável; `test_audit_events.py` também comprova ator nas escritas HTTP atuais e leitura explícita do catálogo/estoque sem adicionar payload clínico aos eventos. Fluxos futuros ainda precisam da mesma verificação. |
