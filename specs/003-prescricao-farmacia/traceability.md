...
> Baseline técnica: `master` em `d78e668f75253aa59295a2bf4d5d8ad31517cf5a`, 2026-09-15.  
...
| **RF-RX-13 — Auditoria e eventos** | **Parcial** | `events.py` emite payload técnico pós-commit; `prescription.created` e `stock.low` ativos. O catálogo registra `ACCESS` somente para `Drug` efetivamente renderizados e as escritas HTTP de criação/edição ficam atribuídas ao ator autenticado; estoque/lotes também geram `ACCESS`. `test_stock_audit_pagination.py` prova que uma página de estoque com paginação audita somente os `StockItem` e `Lot` realmente renderizados e só registra o item restante após abrir a segunda página. `tests/test_audit_events.py` confere superfícies e nomes de eventos; `tests/test_event_contract.py` exige que os campos efetivamente emitidos de `prescription.created` e `stock.low` coincidam exatamente com `contracts/events.asyncapi.yaml`, admitindo apenas `type=prescription_event_handler` como envelope técnico do Django Channels fora do payload AsyncAPI. | T-RX-12: `prescription.validated` e `medication.dispensed`, além de leitura/escrita das futuras telas de prescrição, validação e dispensação. |
...
| **RNF-RX-04 — Minimização de PHI em logs/eventos** | **Parcial** | Eventos atuais têm allowlist técnico; campos textuais sensíveis são excluídos do auditlog onde aplicável. Leituras de catálogo/estoque são registradas somente para objetos efetivamente renderizados, incluindo paginação comprovada para ambos. `test_event_contract.py` impede drift de campos entre runtime e AsyncAPI para os eventos atualmente emitidos e formaliza que a chave `type` é somente roteamento Channels, não conteúdo do evento. Fluxos futuros ainda precisam da mesma verificação. |
...
- RBAC/ABAC e negação Gherkin/E2E: `test_permissions.py`, `test_selectors.py`, `test_acceptance_traceability.py`, `specs/003-prescricao-farmacia/features/authorization.feature`, `tests/e2e/prescription_authorization.spec.js`.
- Estoque/ledger: `test_stock.py`, `test_stock_identity_db_guards.py`.
- Concorrência PostgreSQL: `test_stock_concurrency.py`.
- Auditoria/eventos: `test_audit_events.py`, `test_stock_audit_pagination.py`, `test_event_contract.py`.
