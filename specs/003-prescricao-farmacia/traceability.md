...
| **RNF-RX-04 — Minimização de PHI em logs/eventos** | **Parcial** | Eventos atuais têm allowlist técnico; campos textuais sensíveis são excluídos do auditlog onde aplicável. `test_event_contract.py` impede drift de campos entre runtime e AsyncAPI para os eventos atualmente emitidos e formaliza que a chave `type` é somente roteamento Channels, não conteúdo do evento. Fluxos futuros ainda precisam da mesma verificação. |
