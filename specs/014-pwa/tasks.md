...
- [ ] **T-PWA-07 — BLOCKED / piloto clínico externo.** Implementar fila idempotente de sincronização somente para um fluxo piloto explicitamente aprovado. RF-PWA-03. A infraestrutura local protegida está pronta, mas nenhuma mutação clínica/farmacêutica será habilitada offline até uma spec dona do fluxo aprovar dados permitidos, política de conflito, idempotência de servidor, UX de reconciliação e critérios de segurança. A Spec 003 permanece `network-only` por decisão explícita.
