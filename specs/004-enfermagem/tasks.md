...
- [ ] **T-NUR-06 — PRÓXIMA** Implementar idempotência server-side para sinais vitais por `idempotency_key`. RF-NUR-06/07.
...
- [ ] **T-NUR-13 — BLOCKED até implementação** Criar testes unitários/views/security, incluindo RBAC + ABAC, imutabilidade, idempotência e negações sem PHI. Os incrementos T-NUR-03/04/05 já cobrem invariantes de modelo, permissões, form e superfícies online; a tarefa só fecha com idempotência/offline/administração implementadas.
