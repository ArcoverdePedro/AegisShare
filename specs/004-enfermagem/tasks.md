...
- [ ] **T-NUR-05 — PRÓXIMA** Implementar correção append-only via `replaces` e impedir edição/exclusão destrutiva pela UI. RF-NUR-05.
- [ ] **T-NUR-06 — BLOCKED até T-NUR-05** Implementar idempotência server-side para sinais vitais por `idempotency_key`. RF-NUR-06/07.
...
- [ ] **T-NUR-13 — BLOCKED até implementação** Criar testes unitários/views/security, incluindo RBAC + ABAC, imutabilidade, idempotência e negações sem PHI. Os incrementos T-NUR-03/04 já cobrem invariantes de modelo, permissões, form e superfícies online; a tarefa só fecha com idempotência/offline/administração implementadas.
