...
from ..services import order_exam


@override_settings(STORAGES=TEST_STORAGES, SECURE_SSL_REDIRECT=False)
class RisTests(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.admin = make_user("ris-admin", role="ADM")
        cls.admin.is_superuser = True
        cls.admin.save(update_fields=["is_superuser"])
        cls.user = make_user("ris-user", role="FUNC")
        cls.other = make_user("ris-other", role="FUNC")
        cls.permissions = Permission.objects.filter(
            content_type__app_label="ris", codename__in=["view_orders", "order_exam"]
        )
        for user in (cls.user, cls.other):
            user.user_permissions.add(*cls.permissions)
        cls.patient = Patient.objects.create(
            identifier_type="OTHER",
            identifier="RIS-SYNTH",
            full_name="Paciente sintético RIS",
        ...
        cls.exam = ImagingExam.objects.create(code="RIS-PRIVATE", name="Exame sintético imagem")
        cls.url = reverse("ris:order_create", args=[cls.encounter.pk])
        cls.list_url = reverse("ris:order_list")

    def setUp(self):
        self.client.force_login(self.user)

    def data(self, **changes):
        return {"exam": self.exam.pk, "operation_key": uuid.uuid4(), **changes}
    ...
    def test_snapshot_retry_and_new_operation(self):
        data = self.data(requested_by=self.other.pk, exam_name="FORGED", requested_at="1990-01-01")
        response = self.client.post(self.url, data)
        self.assertEqual(response.status_code, 302)
        item = ImagingOrder.objects.get()
        original = item.requested_at
        self.assertEqual(item.requested_by, self.user)
        self.assertEqual(item.exam_name, self.exam.name)
        self.exam.name, self.exam.code, self.exam.active = "Changed", "Changed", False
        self.exam.save()
        self.assertEqual(self.client.post(self.url, data).url, response.url)
        item.refresh_from_db()
        self.assertEqual(item.requested_at, original)
        self.assertEqual(item.exam_name, "Exame sintético imagem")
        ...
        self.assertEqual(ImagingOrder.objects.count(), 2)

    def test_conflicting_key_does_not_reveal_original(self):
        item = self.order()
        other_exam = ImagingExam.objects.create(code="OTHER", name="Other")
        response = self.client.post(
            self.url, self.data(operation_key=item.operation_key, exam=other_exam.pk)
        )
    ...
    def test_revocation_and_inactive_patient_hide_history(self):
        item = self.order()
        detail = reverse("ris:order_detail", args=[item.pk])
    ...
    def test_closed_encounter_blocks_new_and_retry_but_preserves_read(self):
        ...
        self.assertEqual(
            self.client.get(reverse("ris:order_detail", args=[item.pk])).status_code, 200
        )
        self.assertEqual(ImagingOrder.objects.count(), 1)
    ...
    def test_capabilities_and_internal_role_are_required(self):
        ...
        self.user.user_permissions.add(
            Permission.objects.get(content_type__app_label="ris", codename="view_orders")
        )
        self.assertEqual(self.client.get(self.list_url).status_code, 200)
        self.assertEqual(self.client.post(self.url, self.data()).status_code, 403)
        self.user.user_permissions.set(self.permissions.filter(codename="order_exam"))
        self.assertEqual(self.client.post(self.url, self.data()).status_code, 403)
        outsider = make_user("ris-client", role="CLI")
    ...
    def test_invalid_form_preserves_key_and_active_get_choices(self):
        inactive = ImagingExam.objects.create(code="INACTIVE", name="Inactive", active=False)
        self.assertNotContains(self.client.get(self.url), inactive.name)
        key = uuid.uuid4()
        response = self.client.post(self.url, self.data(exam=uuid.uuid4(), operation_key=key))
        ...
        self.assertEqual(ImagingOrder.objects.count(), 0)
    ...
    def test_audit_failure_rolls_back_mutation_and_blocks_reads(self):
        ...
        self.assertFalse(ImagingOrder.objects.exists())
        item = self.order()
        with patch(
            "apps.clinical.ris.views.accessed.send", side_effect=DatabaseError("audit unavailable")
        ):
            for url in (self.url, self.list_url, reverse("ris:order_detail", args=[item.pk])):
                response = self.client.get(url)
                self.assertEqual(response.status_code, 503)
                self.assertNotContains(response, self.patient.full_name, status_code=503)
        with patch(
            "apps.clinical.ris.services.accessed.send",
    ...
    def test_append_only_protect_and_catalog_validation(self):
        ...
        with self.assertRaises(ProtectedError):
            self.exam.delete()
        with self.assertRaises(ValidationError):
            ImagingExam.objects.create(code="   ", name="Name")
        exam = ImagingExam.objects.create(code=" trimmed ", name=" Name ")
        self.assertEqual((exam.code, exam.name), ("trimmed", "Name"))
        with self.assertRaises(ValidationError):
            ImagingExam.objects.create(code="trimmed", name="Duplicate")
        self.assertNotIn(ImagingOrder, admin.site._registry)
        catalog_admin = admin.site._registry[ImagingExam]
        request = RequestFactory().get("/")
        request.user = make_user("ris-client-admin", role="CLI")
    ...
    def test_audit_minimizes_content_and_records_actor(self):
        item = self.order()
        self.client.get(reverse("ris:order_detail", args=[item.pk]))
        logs = LogEntry.objects.filter(content_type__app_label="ris")
        serialized = str(list(logs.values("changes", "object_repr", "additional_data")))
        for marker in (self.patient.full_name, self.exam.code, self.exam.name):
    ...
    def test_pagination_audits_only_visible_records_without_n_plus_one(self):
        ...
        LogEntry.objects.filter(
            content_type__app_label="ris", action=LogEntry.Action.ACCESS
        ).delete()
    ...
    def test_privacy_headers_cover_csrf_methods_missing_and_redirects(self):
        ...
        responses = [
            ...
            self.client.get(reverse("ris:order_detail", args=[uuid.uuid4()])),
        ]
        self.client.logout()
