...
def transfer_patient(
    *, admission_id, destination_bed_id, actor, operation_key,
    transferred_at=None, reason="",
):
    ...
    try:
        with transaction.atomic():
            ...
            admission = Admission.objects.select_for_update().select_related(
                "encounter__patient"
            ).get(pk=admission_id)
            encounter = Encounter.objects.select_for_update().select_related(
                "patient"
            ).get(pk=admission.encounter_id)
            admission.encounter = encounter
            current = BedOccupancy.objects.select_for_update().select_related(
                "bed__location"
            ).filter(admission=admission, ended_at__isnull=True).first()
            destination = Bed.objects.select_for_update().select_related(
                "location"
...
def discharge_patient(
    *, admission_id, disposition, actor, operation_key,
    discharged_at=None, reason="",
):
    operation_key = _operation_key(operation_key)
    discharged_at = discharged_at or timezone.now()
    try:
        with transaction.atomic():
            existing = Discharge.objects.select_for_update().filter(
                operation_key=operation_key
            ).first()
            if existing:
                return _existing_discharge(
                    existing, admission_id, disposition, actor
                )
