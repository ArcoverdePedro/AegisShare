...
def _apply_file_filters(queryset, request):
    filters = {
        "nome_arquivo__icontains": request.GET.get("nome", "").strip(),
        "dono_arquivo__username__icontains": request.GET.get("dono", "").strip(),
        "mime_type__icontains": request.GET.get("tipo", "").strip(),
        "tags__name__icontains": request.GET.get("tag", "").strip(),
    }
    ...
    if scope == "meus":
        ...
        queryset = queryset.filter(usuarios_permitidos=request.user).exclude(
            dono_arquivo=request.user
        )

    for param, lookup in (
        ("data_min", "data_adicionado__date__gte"),
        ("data_max", "data_adicionado__date__lte"),
    ):
        value = request.GET.get(param, "").strip()
        if value:
            try:
                queryset = queryset.filter(
                    **{lookup: datetime.strptime(value, "%d/%m/%Y").date()}
                )
            except ValueError:
                pass
    ...
    ordering_map = {
        "tamanho_menor": "tamanho_arquivo",
        "tamanho_maior": "-tamanho_arquivo",
        "nome": "nome_arquivo",
    }
...
def buscar_cliente(request):
    ...
    return JsonResponse(
        [{"id": str(user.id), "nome": user.username} for user in clientes], safe=False
    )
...
def buscar_funcionario(request):
    ...
    return JsonResponse(
        [{"id": str(user.id), "nome": user.username} for user in users], safe=False
    )
...
def upload(request):
    if request.user.is_client():
        messages.error(
            request, "Clientes enviam arquivos pelas solicitacoes de documentos."
        )
    ...
    if request.method == "POST" and form.is_valid():
        owner = get_object_or_404(
            CustomUser, id=form.cleaned_data["cliente_id"], nivel_permissao="CLI"
        )
        try:
...
def file_detail(request, file_id):
    ...
    can_share = file.user_pode_compartilhar(request.user)
    can_edit = file.user_pode_alterar(request.user)
    share_candidates = CustomUser.objects.none()
    if can_share:
        share_candidates = (
            CustomUser.objects.filter(is_active=True)
            .exclude(id=request.user.id)
            .exclude(file_access_grants__arquivo=file)
            .order_by("username")
        )
    ...
    return render(
        ...
        "arquivos/file_detail.html",
        {
            ...
            "shared_links": (
                file.shared_links.filter(revoked_at__isnull=True) if can_share else []
            ),
            "share_candidates": share_candidates,
            "can_share": can_share,
            "can_edit": can_edit,
            "new_shared_link": request.session.pop("new_shared_link", None),
        },
    )
...
def add_file_version(request, file_id):
    ...
    if form.is_valid():
        try:
            create_new_version(
                file=file, uploaded_file=form.cleaned_data["arquivo"], actor=request.user
            )
...
def share_file_access(request, file_id):
    ...
    recipient = get_object_or_404(
        CustomUser, id=request.POST.get("usuario_id"), is_active=True
    )
    try:
        _, created = grant_access(file=file, recipient=recipient, actor=request.user)
        messages.success(
            request, "Acesso concedido." if created else "O usuario ja possuia acesso."
        )
...
@login_required
@require_POST
def update_file_tags(request, file_id):
    file = get_accessible_file(request.user, file_id)
    if not file or not file.user_pode_alterar(request.user):
        return HttpResponseForbidden("Sem permissao.")
    names = {
        name.strip()[:50]
        for name in request.POST.get("tags", "").split(",")
        if name.strip()
    }
    tags = [Tag.objects.get_or_create(name=name)[0] for name in sorted(names)[:20]]
    file.tags.set(tags)
    messages.success(request, "Tags atualizadas.")
    return redirect("file_detail", file_id=file.id)
...
def create_file_link(request, file_id):
    ...
    if form.is_valid():
        try:
            ...
            messages.success(
                request,
                "Link criado. Copie-o agora; o token nao fica armazenado em texto puro.",
            )
    else:
...
def shared_link(request, token):
    candidate = _public_link(token)
    if not candidate or not candidate.is_active or candidate.file.deleted_at:
        raise Http404

    if candidate.password_hash and request.method != "POST":
        return render(
            request,
            "arquivos/shared_link.html",
            {"link": candidate, "token": token, "needs_password": True},
        )

    try:
        link = resolve_shared_link(
            token,
        ...
        return render(
            request,
            "arquivos/shared_link.html",
            {"link": candidate, "token": token, "needs_password": True, "error": str(exc)},
            status=403,
        )
    ...
    return render(
        request,
        "arquivos/shared_link.html",
        {"link": link, "token": token, "authorized": True},
    )


def _authorized_public_link(request, token, *, for_download=False):
    candidate = _public_link(token)
    if not candidate or not candidate.is_active:
        raise Http404
    password_verified = bool(
        candidate.password_hash
        and request.session.get(f"shared-link-auth:{candidate.id}")
    )
    if candidate.password_hash and not password_verified:
        raise Http404
    try:
        if for_download:
            return consume_download(token, password_verified=password_verified)
        return resolve_shared_link(
            token, for_download=False, password_verified=password_verified
        )
    except SharedLinkError as exc:
...
def shared_link_preview(request, token):
    link = _authorized_public_link(request, token)
...
def shared_link_download(request, token):
    link = _authorized_public_link(request, token, for_download=True)
    accessed.send(link.file.__class__, instance=link.file)
...
def add_file_comment(request, file_id):
    ...
    if form.is_valid():
        FileComment.objects.create(
            file=file, user=request.user, content=form.cleaned_data["content"]
        )
        recipients = {file.dono_arquivo, *file.usuarios_permitidos.all()}
...
@login_required
def trash(request):
    files = files_for_user(request.user, include_deleted=True).filter(
...
def restore_file(request, file_id):
    ...
    if not file:
        raise Http404
    if not (request.user.is_admin() or file.dono_arquivo_id == request.user.id):
        return HttpResponseForbidden("Sem permissao.")
    file.restore()
...
def workspaces(request):
    if request.user.is_admin():
        qs = Workspace.objects.select_related("cliente", "created_by").prefetch_related(
            "folders"
        )
    else:
        qs = (
            Workspace.objects.filter(Q(cliente=request.user) | Q(members=request.user))
    ...
    return render(
        request,
        "arquivos/workspaces.html",
        {"workspaces": qs, "workspace_form": workspace_form, "folder_form": folder_form},
    )
...
def document_requests(request):
    if request.user.is_client():
        qs = DocumentRequest.objects.filter(recipient=request.user).prefetch_related(
            "items__fulfilled_by_file"
        )
    else:
        qs = DocumentRequest.objects.filter(
            Q(created_by=request.user) | Q(recipient=request.user)
        ).select_related("recipient").prefetch_related("items__fulfilled_by_file")
        ...
        if request.method == "POST" and form.is_valid():
            with transaction.atomic():
                ...
                DocumentRequestItem.objects.bulk_create(
                    [
                        DocumentRequestItem(request=obj, label=label)
                        for label in form.cleaned_data["items"]
                    ]
                )
    ...
    return render(
        request, "requests/document_requests.html", {"requests": qs, "form": form}
    )
...
def fulfill_request_item(request, item_id):
    item = get_object_or_404(
        DocumentRequestItem.objects.select_related(
            "request", "request__recipient", "request__created_by"
        ),
        id=item_id,
    ...
    try:
        ...
        messages.error(request, f"Nao foi possivel enviar o documento: {exc}")
        return redirect("document_requests")
