...
urlpatterns = [
    ...
    path("auditoria/", dashboard.audit_log, name="audit_log"),
    ...
    path(
        "arquivos/<int:file_id>/versoes/<uuid:version_id>/preview/",
        files.file_preview,
        name="file_version_preview",
    ),
    path(
        "arquivos/<int:file_id>/versoes/<uuid:version_id>/download/",
        files.file_download,
        name="file_version_download",
    ),
    path(
        "arquivos/<int:file_id>/versoes/nova/",
        files.add_file_version,
        name="add_file_version",
    ),
    path("arquivos/<int:file_id>/tags/", files.update_file_tags, name="update_file_tags"),
    path(
        "arquivos/<int:file_id>/acessos/novo/",
        files.share_file_access,
        name="share_file_access",
    ),
    path(
        "arquivos/<int:file_id>/acessos/<uuid:user_id>/revogar/",
        files.revoke_file_access,
        name="revoke_file_access",
    ),
    path(
        "arquivos/<int:file_id>/links/novo/",
        files.create_file_link,
        name="create_file_link",
    ),
    path("links/<uuid:link_id>/revogar/", files.revoke_file_link, name="revoke_file_link"),
    path(
        "arquivos/<int:file_id>/comentarios/",
        files.add_file_comment,
    ...
    path(
        "solicitacoes/item/<int:item_id>/enviar/",
        files.fulfill_request_item,
        name="fulfill_request_item",
    ),
    path("notificacoes/", files.notifications, name="notifications"),
    path(
        "notificacoes/lidas/",
        files.mark_notifications_read,
    ...
    path(
        "seguranca/sessoes/<int:session_id>/revogar/",
        security.revoke_session,
        name="revoke_session",
    ),
    path(
        "seguranca/sessoes/revogar-outras/",
        security.revoke_other_sessions,
        name="revoke_other_sessions",
    ),
    path("seguranca/tokens/novo/", security.create_token, name="create_api_token"),
    path(
        "seguranca/tokens/<uuid:token_id>/revogar/",
